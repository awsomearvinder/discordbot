"""
A module to assign roles based on activity.
"""
import aiosqlite
import pytimeparse
...
class Configure(commands.Cog):
    """
    This Cog is used in order to do configuration
    for other cogs.
    """
    ...
    async def configure(self, _):
        """
        Stub for configure command
        """

    @configure.command(name="ActivityTracking")
    async def activity_tracking(self, msg: Message):
        """
        Add a rule based on activity tracking.
        """
        if not msg.guild:
            await msg.reply(
        ...
        await msg.reply("What channel(s) would you like to apply this rule for?")
        ...
        async with aiosqlite.connect(self.bot.db_path) as database:
            ...
            print(activity_tracking_channels)
